...
- `filter_lang(text: str, lang: str, *, keep_fences: bool = False) -> str` — 純粋関数、I/O なし
...
| # | テスト名 | 概要 | lang | 検証内容 |
|---|---------|------|------|---------|
| 8 | `test_en_ja_pair_keep_en` | en+jaペア | `"en"` | en本文のみ残存、jaブロック除去 |
| 9 | `test_en_ja_pair_keep_ja` | en+jaペア | `"ja"` | ja本文のみ残存、enブロック除去 |
...
| 16 | `test_lang_block_containing_code_fence_kept` | langブロック内にコードフェンス (keep) | 内容（コードフェンス含む）がwrapperなしで残る |
...
| # | テスト名 | 概要 |
|---|---------|------|
| 40 | `test_full_document_en` | 実文書相当の入力を lang=en でフィルタ。en内容+共有コンテンツがwrapperなしで残り、ja内容が消える |
| 41 | `test_full_document_ja` | 同上を lang=ja で |
| 42 | `test_full_document_both` | 同上を lang=both で。入力と完全一致 |
...
9. Phase 9 (#35-#39): CLI層
10. Phase 10 (#40-#42): 実文書相当の統合テスト
